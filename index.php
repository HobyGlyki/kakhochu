	...
	<form action=" " method="post">
		<?php for($i=0;$i<count($query);$i++) { ?>
	        <div class="task">
				...
				<?php
if (($query[$i]['task_complete']==0) and $ched== null)
{
	$chedn ="не сделанно ";
	$ched = " ";
}
elseif(($query[$i]['task_complete']==1) and $ched== null){
	 $ched = "checked";
         $chedn ="сделанно ";
       } 
echo $chedn;
?>	
                <input type="checkbox" name=<?php echo "check".$i?> <?php echo $ched;?>="0" value="1">
            </div>
...
#echo ( $update_sql1);
$upd = $db->update($name, $set1, $where1);}
